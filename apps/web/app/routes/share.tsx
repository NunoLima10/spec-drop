...
export default function Share() {
  ...
  if (state.status === "loading") {
    return (
      <main
        aria-busy="true"
        className="min-h-screen bg-[#05060f] px-6 py-12 text-[#d8ecf8]"
      >
        <p className="mx-auto max-w-3xl text-[#9da7ba]">Loading...</p>
      </main>
    );
  }
  ...
  if (state.status === "error") {
    return (
      <main className="min-h-screen bg-[#05060f] px-6 py-12 text-white">
        <section className="mx-auto max-w-3xl">
          <h1 className="font-semibold text-3xl">Document unavailable</h1>
          <p className="mt-3 text-[#9da7ba]">{state.message}</p>
        </section>
      </main>
    );
  }
  ...
  const title = state.share.title || "Untitled Markdown";

  async function handleCopyMarkdown() {
    const didCopy = await copyMarkdownToClipboard(state.share.content);
    setCopyStatus(didCopy ? "Copied" : "Copy failed");
  }

  function handleDownloadMarkdown() {
    const fileName = `${toMarkdownFileName(title)}.md`;
    const blob = new Blob([state.share.content], {
            ...
            <section className="min-w-0 max-w-3xl">
              <header className="mb-6 pb-4">
                ...
                <ShareMetadata
                  ...
                  currentViews={state.share.currentViews}
              ...
              {previewMode === "code" ? (
                <MarkdownSource content={state.share.content} />
              ) : (
